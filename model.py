...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    words = list(specials)
    words.extend([word for sentence in sentences for word in sentence.split()])
    words = list(dict.fromkeys(words))
    id={key:idx for idx, key in enumerate(words)} 
...
def build_id_to_token_vocab(token_to_id):
    ...
    id = {id:token for token,id in token_to_id.items()}
    return id
    pass
